(function (r) {
    ...
    r.fn.centerWithOffset = function (topoffset) {
        this.parent().css("display", "inline-table");
        var t = this.parent().css("top") + topoffset;
        var l = this.parent().css("left");

        this.css("position", "absolute");
        //this.css("top", (((topoffset)) + this.parent().scrollTop() )+ "px");
        this.css("top", (topoffset * 1.25) + "px");
        this.css("left", ((this.parent().width() - this.outerWidth()) / 2) + this.parent().scrollLeft() + "px");
        //console.log(this.css("top"));
        //console.log(topoffset);
        //console.log((topoffset + 100) + "px");
        return this;
    }
    r.fn.qrcode2 = function (h) {
        var s;
        function u(a) {
            this.mode = s;
            this.data = a
        ...
        return this.each(function () {
            ...
            $(c).center();
            if (h.withText) {
                var topOffset = c.height;

                var label = document.createElement("label");
                label.id = "qrText";
                label.innerText = h.text;
                //$(label).css('justify-self', 'center');
                $(label).css('max-width', '200px');
                $(label).css('word-break', 'break-all');
                //$(label).css('overflow', 'hidden');
                jQuery(label).appendTo(this)
                $(label).centerWithOffset(topOffset);
